...
frappe.ui.form.ControlGeolocation = class ControlGeolocation extends frappe.ui.form.ControlData {
	...
	make_map(value) {
		...
		try {
			...
			$(`#${this.map_id}`).html(`<div class="map-error">Map failed to load</div>`);
			return;
		}
		if (this.disabled) {
	...
	bind_leaflet_data(value) {
		...
		if (!value || value.trim() === '') {
			console.warn("No location value provided.");
			return;
		}

		let geojson_data;
		try {
			geojson_data = JSON.parse(value);
		} catch (error) {
			console.error("Invalid GeoJSON data:", value, error);
			return;
		}

		if (!geojson_data || (Array.isArray(geojson_data.features) && geojson_data.features.length === 0)) {
			console.warn("Empty GeoJSON, nothing to show on map.");
			return;
		}
