...
def fetch_card_data(partner_id=None, state_id=None, district_id=None, gp_id=None, block_id=None, creche_id=None, year=None, month=None, supervisor_id=None, cstart_date=None, cend_date=None, c_status=None, phases=None, query_type="active_children"):
    ...
    current_user_partner = frappe.db.get_value("User", user, "partner")
    ...
    state_query = """ 
        ...
        WHERE parent = %s
    """
    user_geo = frappe.db.sql(state_query, user, as_dict=True)

    state_ids = [str(s["state_id"]) for s in user_geo if s.get("state_id")]
    district_ids = [str(s["district_id"]) for s in user_geo if s.get("district_id")]
    block_ids = [str(s["block_id"]) for s in user_geo if s.get("block_id")]
    gp_ids = [str(s["gp_id"]) for s in user_geo if s.get("gp_id")]

    village_id = data.get("village_id") or None

    creche = data.get("creche_id") or None
    ...
    WHERE 
        (%(partner_id)s IS NULL OR tc.partner_id = %(partner_id)s) AND
        (%(state_id)s IS NULL OR tc.state_id = %(state_id)s) AND
        (%(district_id)s IS NULL OR tc.district_id = %(district_id)s) AND
        (%(block_id)s IS NULL OR tc.block_id = %(block_id)s) AND
        (%(gp_id)s IS NULL OR tc.gp_id = %(gp_id)s) AND
        (%(supervisor_id)s IS NULL OR tc.supervisor_id = %(supervisor_id)s) AND
        (%(creche_id)s IS NULL OR tc.name = %(creche_id)s) AND
        (%(c_status)s IS NULL OR tc.creche_status_id = %(c_status)s) AND
        (%(phases)s IS NULL OR FIND_IN_SET(tc.phase, %(phases)s))
    ...
    -- exited children this month --
        ...
        WHERE 
            cee.date_of_exit IS NOT NULL AND
            YEAR(cee.date_of_exit) = %(year)s AND  
            MONTH(cee.date_of_exit) = %(month)s AND  
            (%(partner_id)s IS NULL OR cee.partner_id = %(partner_id)s) AND  
            (%(state_id)s IS NULL OR cee.state_id = %(state_id)s) AND  
            (%(district_id)s IS NULL OR cee.district_id = %(district_id)s) AND  
            (%(block_id)s IS NULL OR cee.block_id = %(block_id)s) AND  
            (%(gp_id)s IS NULL OR cee.gp_id = %(gp_id)s) AND 
            (%(creche_id)s IS NULL OR cee.creche_id = %(creche_id)s) AND
            (%(supervisor_id)s IS NULL OR cr.supervisor_id = %(supervisor_id)s) AND
            (%(c_status)s IS NULL OR cr.creche_status_id = %(c_status)s) AND
            (%(phases)s IS NULL OR FIND_IN_SET(cr.phase, %(phases)s))
            AND (cr.creche_opening_date IS NULL OR ( %(end_date)s IS NOT NULL AND cr.creche_opening_date <= %(end_date)s ))
            AND
            (
                (%(cstart_date)s IS NULL AND %(cend_date)s IS NULL) OR 
                (cr.creche_opening_date BETWEEN %(cstart_date)s AND %(cend_date)s)
            );
