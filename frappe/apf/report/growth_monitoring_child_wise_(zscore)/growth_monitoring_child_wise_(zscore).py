...
def calculate_z_score(value, M, L, S):
    ...
    ratio = value / M
    ratio_to_L = math.pow(ratio, L)
    numerator = ratio_to_L - 1
    denominator = S * L
    z_score = numerator / denominator

    # return z_score
    truncated_z = float(f"{z_score:.3f}") 
    return round(truncated_z,2)
...
def get_columns():
    columns = [
        ...
        # {"label": "Weight for Age", "fieldname": "weight_for_age_status", "fieldtype": "Data", "width": 150},
        # {"label": "Weight for Height", "fieldname": "weight_for_height_status", "fieldtype": "Data", "width": 150},
        # {"label": "Weight for Age Z-Score", "fieldname": "weight_for_age_zscore", "fieldtype": "Float", "width": 150, "precision": 2},
        # {"label": "Weight for Height Z-Score", "fieldname": "weight_for_height_zscore", "fieldtype": "Float", "width": 150, "precision": 2},
    ]
...
def execute(filters=None):
    ...
    for row in data:
        ...
        if height is not None and weight is not None and weight > 0 and height > 0 and gender_id in ('1', '2'):
            try:
                height_rounded = round(float(height))
                weight = float(weight)
