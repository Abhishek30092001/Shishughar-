...
def execute(filters=None):
    ...
    variable_columns = get_variable_columns(selected_level)
    ...
    # Get data synchronously since parallel execution causes DB connection issues
    data_part1 = get_summary_data_part1(filters)
    data_part2 = get_summary_data_part2(filters)
    
    # Combine the results
    data = combine_data(data_part1, data_part2)
...
def get_variable_columns(selected_level):
    variable_columns = []
    if selected_level == "1":
...
def get_summary_data_part1(filters):
    """First part of the query - gets basic enrollment and attendance data"""
    params = prepare_query_params(filters)
    where_clause = prepare_where_clause(filters, params)
    group_by_field = prepare_group_by(filters)
    ...
    FROM (
        SELECT 
            ...
            c.creche_id,
            IFNULL(ec.child_count, 0) AS e_children,  -- Changed from ec.e_children to ec.child_count
        ...
        LEFT JOIN (
            SELECT hf.creche_id, COUNT(hhc.hhcguid) AS child_count  -- Changed column name to child_count
        ...
        LEFT JOIN(
            ...
            FROM (
                ...
                FROM (
                    SELECT cee.creche_id, cee.childenrollguid, chatt.days_attended, chatt.eligible_open_days  
                    FROM `tabChild Enrollment and Exit` cee
                    LEFT JOIN (
                        SELECT cal.childenrolledguid, COUNT(CASE WHEN cal.attendance = 1 THEN 1 END) AS days_attended, 
...
def get_summary_data_part2(filters):
    """Second part of the query - gets attendance statistics"""
    params = prepare_query_params(filters)
    where_clause = prepare_where_clause(filters, params)
    group_by_field = prepare_group_by(filters)
    ...
    FROM (
        ...
        LEFT JOIN (
            SELECT creche_id, MIN(daily_attendance) AS min_att, 
                   CEIL((SUM(daily_attendance)/ COUNT(*))) AS mean_att, 
                   MAX(daily_attendance) AS max_att
            FROM (
        ...
        LEFT JOIN (
            SELECT COUNT(*) as open_days_count, ca.creche_id
            FROM `tabChild Attendance` ca
...
def prepare_query_params(filters):
    ...
    # Creche opening date range logic
    ...
    params = {
        ...
        "c_status": filters.get("c_status") if filters else None,
...
def prepare_where_clause(filters, params):
    """Prepare the WHERE clause for the queries"""
    conditions = ["1=1"]
    ...
    if filters and filters.get("creche"):
    ...
    if params["cstart_date"] or params["cend_date"]:
        conditions.append("(c.creche_opening_date BETWEEN %(cstart_date)s AND %(cend_date)s)")
    if filters and filters.get("band"):
        conditions.append("(c.band = %(band)s)")
    if filters and filters.get("c_status"):
        conditions.append("(c.creche_status_id = %(c_status)s)")
    if filters and filters.get("creche_status_id"):
...
def prepare_group_by(filters):
    """Prepare the GROUP BY clause based on the selected level"""
    level_mapping = {
        "1": ["tf.partner"],
        "2": ["tf.state"],
...
def combine_data(data_part1, data_part2):
    """Combine data from both queries"""
    combined_data = []
    
    # Create a dictionary for quick lookup of part2 data
    part2_dict = {}
    for row in data_part2:
        key = tuple(row[field] for field in row if field not in ['min_att', 'mean_att', 'max_att', 'avg_attendance_per_day', 
                                                               'avgperenroll', 'avgpereli', 'attendance_percentage', 'band'])
        part2_dict[key] = row
    
    # Merge data from both parts
    for row1 in data_part1:
        key = tuple(row1[field] for field in row1 if field not in ['act_creches', 'e_children', 'enroll_children', 'uni_children',
                                                                 'Total_eligible_open_days', 'Total_days_attended', 'E0', 'G0toL25',
                                                                 '25toL50', '50toL75', '75toL100', 'E100'])
        if key in part2_dict:
            combined_row = {**row1, **part2_dict[key]}
