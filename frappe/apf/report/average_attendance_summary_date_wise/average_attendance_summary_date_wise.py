...
def execute(filters=None):
    ...
    fixed_columns = [
        ...
        #{"label": "Average Attendance per day", "fieldname": "avg_children", "fieldtype": "Int", "width": 250},
...
def get_summary_data(filters=None):
    ...
    if filters.get("phases"):
        ...
        params["phases"] = filters.get("phases")
    ...
    SELECT * FROM (
        SELECT
            ...
            CASE WHEN SUM(tf.codays) = 0 THEN 0 ELSE FORMAT( SUM(tf.ccprs) /  SUM(tf.codays), 2) END AS avg_children,
        ...
        FROM (
            ...
            LEFT JOIN (
                ...
                WHERE (%(partner)s IS NULL OR partner_id = %(partner)s)
